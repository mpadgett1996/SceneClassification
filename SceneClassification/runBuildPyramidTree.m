...
%% DIRECTORY MAPPING
    ...
    for subdir = 1:length(subdir_list)
        ...
        % SVM CREATION
        svmModels{subdir} = TreeBagger(num_trees, pyramid_train, train_labels(subdir,:)');
    end
    ...
    for subdir = 1:length(subdir_list)
        ...
        predict_train = str2double(predict_train);
    ...
    for subdir = 1:length(subdir_list)
        ...
        predict_test = str2double(predict_test);
...
%     for subdir = 1:length(subdir_list)
%         figure(10); hold on; subplot(1, length(subdir_list), subdir);
...
%         figure(11); hold on; subplot(1, length(subdir_list), subdir);
