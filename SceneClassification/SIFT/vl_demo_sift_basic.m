...
%                                                             Run SIFT
% --------------------------------------------------------------------
% Plot only the locations of the features along with the scale
[f,d] = vl_sift(I) ; %f contains the features (location and scale) and d contains the descriptors (representation of region around f in a vector)

hold on ;
perm = randperm(size(f,2)) ;
sel  = perm(1:50) ; %Randomly select 50 features to show
...
%% % Overlay the plot with the descriptor
delete([h1 h2]);
...
%                                                      Custom keypoint
% --------------------------------------------------------------------
% Custom implies that we can define our own location at which to compute the SIFT descriptor

clf ; imagesc(I(1:200,1:200,:)) %First zoom the image
axis equal ; axis off ; axis tight ;
hold on;

fc = [100;100;10;-pi/6] ; %Our point is (100, 100) scale = 10 and orientation = -pi/6 (-30 degrees)
...
% --------------------------------------------------------------------
% We need not provide orientation but can use the dominant orientation
% given by the SIFT descriptor 
fc = [100;100;10;0] ; %Providing orientation value is not useful (since its not going to be used)
